...
# Conditional diffusion network used by Palette.
#
# This file contains the actual diffusion mechanics:
# - construction of the UNet denoiser;
# - beta/gamma noise schedule;
# - forward noising process q(y_t | y_0);
# - reverse denoising process used during restoration;
# - diffusion training loss.
#
# The high-level training loop is implemented in models/model.py.
# For FMI inpainting, this network receives:
#   y_0    : original FMI patch used as target;
...
class Network(BaseNetwork):
    def __init__(self, unet, beta_schedule, module_name='sr3', **kwargs):
        """
        Build the conditional diffusion network.

        The UNet backbone is selected by module_name:
            - "sr3"
            - "guided_diffusion"

        The actual UNet hyperparameters and the diffusion beta schedule are
        provided by the JSON configuration.
        """
    ...
    def set_new_noise_schedule(self, device=None, phase='train'):
        """
        Build and register the diffusion noise schedule for train or test.

        The beta schedule is read from the config, converted into cumulative
        alpha products (gammas), and stored as registered buffers so that they
        move with the model across CPU/GPU devices.

        These buffers are used both for adding noise during training and for
        the reverse denoising process during restoration.
        """
        if device is None:
    ...
    def q_sample(self, y_0, sample_gammas, noise=None):
        ...
        Given a clean target image y_0 and a sampled noise level gamma, produce
        a noisy version y_t by mixing y_0 with Gaussian noise.
        """
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8):
        """
        Reverse diffusion / restoration loop.

        Starting from y_t, repeatedly denoise the image conditioned on y_cond.
        For inpainting, pixels outside the mask are clamped back to y_0 at each
        reverse step, while pixels inside the mask are generated by the model.

        This behavior is appropriate for validation with artificial masks,
        where y_0 is known. For real FMI gap completion, the same logic will
        need to preserve the observed image outside the real gap mask.
        """
        b, *_ = y_cond.shape
